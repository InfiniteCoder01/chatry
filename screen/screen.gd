...
@onready var chat: ChatOverlay = %Chat
...
func _process(_delta: float) -> void:
	...
	if Input.is_action_just_pressed("tool wheel"):
		var tool_wheel := preload("res://screen/tool_wheel/tool_wheel.tscn").instantiate()
		tool_wheel.screen = self
	...
	if Input.is_action_just_pressed("attack"):
		var fireball := preload("res://screen/plushie/moves/fire/fireball.tscn").instantiate()
...
func plushieball(tournament: bool) -> void:
	...
	var court: Court = preload("res://screen/plushieball/court.tscn").instantiate()
